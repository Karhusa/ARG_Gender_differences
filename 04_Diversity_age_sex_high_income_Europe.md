...
```r
hic_eu <- Subset %>%
  filter(World_Bank_Income_Group == "High income", continent == "Europe")
unique(hic_eu$country)
```
Age category (for boxplot)
```r
hic_eu_clean <- hic_eu %>%
  filter(!is.na(age_category), !is.na(sex))
table(hic_eu_clean$sex, useNA = "ifany")

table_df <- hic_eu_clean %>%
...
table_image <- table_df %>%
  ...
  cols_label(
    ...
    male = "Male (n)",
    Total = "Total (n)"
  ) %>% tab_header(title = "High-Income Europe: Sample Distribution by Age and Sex")
...
```r
hic_eu_clean_num <- hic_eu %>%
  filter(!is.na(age_years), !is.na(sex))

sex_table <- hic_eu_clean_num %>% count(sex)
...
sex_gt <- sex_table %>%
  ...
  tab_header(title = "Sex Distribution in High-Income Europe Cohort")
sex_gt
```
## 2.2 Boxplot

```r
plot_df <- hic_eu_clean %>%
  filter(
    !is.na(ARG_div_shan),
    !is.na(sex),
    !is.na(age_category)
...
scale_fill_manual(values = c("Female" = "#8B0000",   # dark red
                             "Male"   = "#003366"))  # dark navy blue

ggplot(plot_df, aes(x = age_category, y = ARG_div_shan, fill = sex)) +
  geom_jitter(
    position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.6),
    size = 1.2, alpha = 0.25, color = "grey20") +
  geom_boxplot(
    width = 0.55,
    outlier.shape = NA,
    alpha = 0.85,
  ...
  labs(
    title = "ARG Diversity by Age Category and Sex (High-Income Europe)",
    x = "Age Category",
    y = expression(Shannon~"ARG diversity"),
    fill = "Sex"
  ) +
  stat_compare_means(
    aes(group = sex),
...
```r
model <- lm( ARG_div_shan ~ sex + age_years, data = hic_eu_clean_num)
summary(model)
```
